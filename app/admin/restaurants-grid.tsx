...
export default function RestaurantsGrid({
  ...
  const [posterCategoryId, setPosterCategoryId] = useState<number | null>(null);
  const [menuCategories, setMenuCategories] = useState<MenuCategory[]>([]);
  const [menuLoading, setMenuLoading] = useState(false);
  const [menuSyncing, setMenuSyncing] = useState(false);
  const [menuError, setMenuError] = useState("");
  const hasPosterCredentials = Boolean(
    activeRestaurant?.tokenPoster && activeRestaurant?.spotId
  );
  ...
  const fetchMenu = async (restaurantId: number) => {
    ...
    try {
      ...
      if (!response.ok) {
        let message = t("Не удалось загрузить меню.", "Menyu yuklab bo'lmadi.");
        try {
          const data = (await response.json()) as { error?: string };
          if (data?.error) message = data.error;
        } catch {
          message = t("Не удалось загрузить меню.", "Menyu yuklab bo'lmadi.");
        }
  ...
  const syncMenu = async (
    ...
    try {
      const response = await fetch(
        `/api/restaurants/${restaurantId}/menu/sync`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      if (!response.ok) {
        let message = t(
          ...
          "Menyu sinxronlash amalga oshmadi."
        );
        try {
          const data = (await response.json()) as { error?: string };
          if (data?.error) message = data.error;
                  ...
                  <div className="p-6">
                    ...
                    <div className="mt-6 flex flex-col gap-3 text-sm">
                      <button
                        type="button"
                        onClick={() => {
                          if (!canChangeRestaurantStatus) return;
                          setStatusMenuOpen(true);
                        }}
                        className={`w-full rounded-2xl border px-4 py-3 text-left transition ${
                          canChangeRestaurantStatus
                        ...
                        disabled={!canChangeRestaurantStatus}
                      >
                      ...
                      <button
                        ...
                        title={
                          canEditRestaurants ? "" : t("Нет прав", "Ruxsat yo'q")
                        }
                    ...
                    <div className="mt-6 rounded-2xl border border-white/30 bg-white/5 p-4">
                      <div className="flex flex-wrap items-center justify-between gap-2 text-xs uppercase tracking-[0.2em] text-white/70">
                        <span>{t("Список категории", "Kategoriya ro'yxati")}</span>
                        {activeRestaurant.integrationType !== "1c" && (
                          <div className="flex items-center gap-2 text-[10px] uppercase tracking-[0.2em]">
                            <button
                              type="button"
                              onClick={() =>
                                syncMenu(activeRestaurant.id, "categories")
                              }
                              disabled={menuSyncing || !hasPosterCredentials}
                              title={
                                hasPosterCredentials
                                  ? ""
                                  : t(
                                      "Укажите Token Poster и Spot ID для синхронизации",
                                      "Sinxronlash uchun Poster Token va Spot ID kiriting"
                                    )
                              }
                              className="rounded-full border border-white/30 bg-white/10 px-2 py-1 text-white/70 transition hover:bg-white/20 disabled:cursor-not-allowed disabled:opacity-50"
                            >
                            ...
                            <button
                              type="button"
                              onClick={() =>
                                syncMenu(activeRestaurant.id, "items")
                              }
                              disabled={menuSyncing || !hasPosterCredentials}
                              title={
                                hasPosterCredentials
                            ...
                            <button
                              type="button"
                              onClick={() => syncMenu(activeRestaurant.id, "all")}
                              disabled={menuSyncing || !hasPosterCredentials}
                              title={
                                hasPosterCredentials
                                  ? ""
                      ...
                      {menuError ? (
                        <div className="mt-3 rounded-2xl border border-red-200/40 bg-red-500/20 px-3 py-2 text-xs uppercase tracking-[0.2em] text-red-100">
                          {menuError}
                        </div>
                      ) : null}
                      {activeRestaurant.integrationType === "1c" ? (
                        <div className="mt-4 text-sm text-white/70">
                          {activeRestaurant.onecBaseUrl
                            ? t(
                                "Показаны товары из 1C (placeholder до подключения).",
                                "1C dan tovarlar ko'rsatilgan (ulanishgacha placeholder)."
                              )
                            : t(
                                "Укажите Base URL и метод авторизации для 1C, чтобы загрузить товары.",
                                "1C uchun Base URL va avtorizatsiya usulini kiriting, tovarlarni yuklash uchun."
                              )}
                        </div>
                      ) : (
                        <>
                          {!activeRestaurant.tokenPoster ||
                          !activeRestaurant.spotId ? (
                            <div className="mt-2 text-sm text-white/70">
                              {t(
                                "Укажите Token Poster и Spot ID, чтобы загрузить Категории и товары. Пока показан шаблон.",
                                "Kategoriyalar va tovarlarni yuklash uchun Poster Token va Spot ID kiriting. Hozircha shablon ko'rsatilgan."
                              )}
                            </div>
                          ) : null}
                          {!posterCategoryId && (
                            <div className="custom-scroll mt-4 grid max-h-40 gap-2 overflow-y-auto text-sm">
                              {menuCategories.map((category) => (
                                <div
                                  ...
                                  tabIndex={0}
                                  onClick={() =>
                                    setPosterCategoryId(category.id)
                                  }
                                  onKeyDown={(event) => {
                                    if (
                                      event.key === "Enter" ||
                                  ...
                                  className={`flex items-center justify-between rounded-xl border border-white/20 bg-white/10 px-3 py-2 text-left transition hover:bg-white/20 ${
                                    category.hidden
                              ...
                              {menuCategories.length === 0 && !menuLoading ? (
                                <div className="rounded-xl border border-white/20 bg-white/10 px-3 py-2 text-white/60">
                                  {t(
                                    "Категории пока не добавлены.",
                                    "Kategoriyalar hozircha qo'shilmagan."
                                  )}
                                </div>
                              ) : null}
                            </div>
                          ...
                          {posterCategoryId ? (
                            <>
                              <div className="mt-4 flex items-center justify-between text-xs uppercase tracking-[0.2em] text-white/70">
                                <span>{t("Товары", "Tovarlar")}</span>
                                <button
                                  type="button"
                                  onClick={() => setPosterCategoryId(null)}
                                  className="rounded-full border border-white/30 bg-white/10 px-2 py-1 text-[10px] uppercase tracking-[0.2em] text-white/70 transition hover:bg-white/20"
                                >
                                  {t("Назад", "Ortga")}
                                </button>
                              </div>
                              <div className="custom-scroll mt-3 grid max-h-52 gap-2 overflow-y-auto text-sm">
                                {(
                                  menuCategories.find(
                                    (category) =>
                                      category.id === posterCategoryId
                                  )?.items ?? []
                                ).map((item) => (
                                  <div
                                    key={item.id}
                                    className={`flex items-center justify-between rounded-xl border border-white/20 bg-white/10 px-3 py-2 ${
                                      item.hidden
                                        ? "text-white/40 opacity-60"
                                        : ""
                                    }`}
                            ...
                            <div className="mt-4 text-sm text-white/70">
                              {t(
                                ...
                                "Tovarlarni ko'rish uchun kategoriya tanlang."
                              )}
                            </div>
                          )}
                        </>
          ...
          >
            <div className="flex items-center justify-between">
              <h4 className="text-lg font-semibold">
                {t("Изменить статус", "Statusni o'zgartirish")}:{" "}
                {activeRestaurant?.status ?? t("Статус", "Status")}
              </h4>
              <button
                type="button"
                onClick={() => setStatusMenuOpen(false)}
            ...
            <div className="mt-4 flex flex-col gap-3 text-sm">
              {["Открыто", "Закрыто"].map((label) => (
                <button
                  key={label}
                  type="button"
                  onClick={() => {
                    if (!activeRestaurant) return;
                    const open = label === "Открыто";
        ...
        >
          ...
          >
            ...
            <div className="mt-6 grid gap-4 text-sm">
              ...
              {renderIntegrationFields(editDraft, (value) =>
                setEditDraft(value as Restaurant)
              )}
              <label className="grid gap-2">
                {t("Адрес", "Manzil")}
                <input
